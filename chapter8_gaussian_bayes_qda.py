...
def generate_dataset(seed=RANDOM_SEED):
    ...
    cov0 = np.array([[2.0, 0.0],
                     [0.0, 2.0]])

    mu1 = np.array([-2.0, 1.0])
    cov1 = np.array([[2.0, 1.0],
                     [1.0, 2.0]])
...
def load_or_generate_dataset():
    ...
    if path.exists():
        print(f"Loading existing dataset: {CSV_FILENAME}")
        return pd.read_csv(path)

    print("CSV not found; generating the same Gaussian dataset.")
    df = generate_dataset()
...
def plot_qda(train_df, holdout_df, model):
    ...
    x1_min = all_df["x1"].min() - 1.0
    x1_max = all_df["x1"].max() + 1.0
    x2_min = all_df["x2"].min() - 1.0
    x2_max = all_df["x2"].max() + 1.0
