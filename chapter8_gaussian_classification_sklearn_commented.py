...
def train_and_record_history(model, X_train, y_train, X_hold, y_hold):
    ...
    ------------------------
    The actual neural-network TRAINING happens inside model.partial_fit(...).

    Internally, scikit-learn performs the equivalent of:

        forward propagation
                ↓
        internal cross-entropy / log-loss
                ↓
        backpropagation through the network
                ↓
        compute gradients dL/dW and dL/db
                ↓
        SGD updates of weights and biases

    In other words, partial_fit() contains the entire training cycle that we
    previously wrote explicitly in NumPy.

    The later calls to predict_proba() and log_loss() are NOT part of training.
    They are measurement only:

        predict_proba()  -> forward pass only
        log_loss()       -> evaluate BCE for plotting/monitoring only

    Those externally computed losses are NOT backpropagated into the model.

    Also note that scikit-learn's MLPClassifier performs its own built-in
    backpropagation.  This is conceptually similar to what autodiff frameworks
    such as PyTorch or TensorFlow accomplish, but MLPClassifier is not exposing
    a general-purpose automatic-differentiation computational graph here.
    """
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        #
        # partial_fit() performs ONE incremental training pass over the
        # supplied training data.  Inside this call, scikit-learn performs:
        #
        #   1. FORWARD PROPAGATION
        #        - compute hidden-layer weighted sums
        #        - apply sigmoid/logistic activation
        #        - compute output probability
        #
        #   2. INTERNAL LOSS CALCULATION
        #        - compute the classifier's training loss
        #
        #   3. BACKPROPAGATION
        #        - propagate error derivatives backward through the network
        #
        #   4. GRADIENT CALCULATION
        #        - compute dL/dW and dL/db for every learned parameter
        #
        #   5. PARAMETER UPDATE
        #        - update weights and biases using mini-batch SGD
        #
        # This one line therefore replaces the hand-written NumPy functions:
        # initialize_network()
        # forward_propagation()
        ...
        if epoch == 1:
            # On the first call only, scikit-learn must be told which output
            # classes are possible so partial_fit() can initialize correctly.
            model.partial_fit(X_train, y_train, classes=classes)
        else:
            model.partial_fit(X_train, y_train)

        # ==============================================================
        # EVERYTHING BELOW THIS POINT IS MEASUREMENT ONLY.
        # NO LEARNING OCCURS HERE.
        # ==============================================================

        # FORWARD PASS ONLY through the UPDATED network.
        #
        # predict_proba() does not change any weights.  It simply propagates
        # X through the current network and returns class probabilities.
        #
        ...
        # Compute BCE / log-loss ONLY so that we can monitor and plot
        # convergence after each epoch.
        #
        # These log_loss() values are NOT fed back into the network and do
        # NOT initiate backpropagation.
